...
import { LayoutDashboard, Users, GitBranch, Wallet, Server, TrendingUp, LogOut, X } from "lucide-react";
...
export function AdminBottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-border/30 bg-background/95 backdrop-blur-xl safe-area-bottom">
      <div className="flex items-center justify-around px-1 py-1.5">
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.path} href={item.path}>
              <div className={`flex flex-col items-center gap-0.5 px-2 py-1 rounded-lg min-w-[48px] ${isActive ? "text-primary" : "text-foreground/35"}`}>
        ...
        <button onClick={handleLogout} className="flex flex-col items-center gap-0.5 px-2 py-1 rounded-lg min-w-[48px] text-foreground/35">
          <LogOut className="h-[18px] w-[18px]" />
          <span className="text-[10px] font-medium leading-tight">退出</span>
        </button>
      </div>
    </nav>
  );
